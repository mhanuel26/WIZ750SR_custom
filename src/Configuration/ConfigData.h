...
struct __network_info {
	...
	uint8_t state;					// WIZ107SR: BOOT(0), OPEN (1), CONNECT (2), UPGRADE (3), ATMODE (4)
...
struct __options {
	...
	uint8_t tcp_rcr_val;
	uint8_t serial_command;			// Serial Command
	uint8_t serial_command_echo;	// Serial Command
	uint8_t serial_trigger[3];		// Serial Command Mode
} __attribute__((packed));
...
struct __user_io_info {
	...
	uint16_t user_dig_type;			// 0: Normal / 1: Pulse Input
...
typedef struct __DevConfig {
	...
	uint8_t fw_ver[3];			// 10진수. Major Version . Minor Version . Maintenance Version
