...
#include "common.h"
//#include "W7500x_board.h"		// change to segcp.h
        ...
        if((cmdnum = parse_SEGCP(treq, param)) != SEGCP_UNKNOWN)
        {
            ...
            if(*param == 0)
            {
                ...
                switch((teSEGCPCMDNUM)cmdnum)
                {
                    ...
                    // Read configuration of Digital Input as Normal or Pulse Measurement Type
                    case SEGCP_NA:
                    case SEGCP_NB:
                    case SEGCP_NC:
                    case SEGCP_ND:
                    ...
                    case SEGCP_TA:
                    	...
                    	tmp_int = USER_IO_A << io_num;					// this might be used for all cases
...
// User GPIOs
...
#ifndef  __USE_GPIOA_INPUT_ONLY__
                    case SEGCP_GA:
#else
                    case SEGCP_GA:
                    	ret |= SEGCP_RET_ERR_INVALIDPARAM;
                    ...
                    case SEGCP_ND:
                    	io_num = (teSEGCPCMDNUM)cmdnum - SEGCP_NA;
                    	tmp_int = atoi((char*)param);
                    	if(tmp_int > DIG_PULSE_IN){
                    		ret |= SEGCP_RET_ERR_INVALIDPARAM;
                    	}
                    	else
                    	{
                    		dig_type = (uint8_t)(tmp_int & 0x01);
                    		set_user_dig_type(USER_IO_SEL[io_num], dig_type);
                    		init_user_io(USER_IO_SEL[io_num]);
                    	}
                    	break;
                    // Set the Frequency Value - This is not implemented
                    //  might be the case when the user wants to output a PWM signal.
                    case SEGCP_TA:
                    case SEGCP_TB:
                    case SEGCP_TC:
                    case SEGCP_TD:
