...
#ifdef _GPIO_DEBUG_
	#include <stdio.h>
//#define _PULSE_DEBUG_		// careful only for push button debug on dev board
#endif
...
static volatile uint32_t pulse_io_a_cnt = 0;
static volatile uint32_t pulse_io_b_cnt = 0;
static volatile uint32_t pulse_io_c_cnt = 0;
static volatile uint32_t pulse_io_d_cnt = 0;

uint32_t get_pulse_cnt(uint16_t GPIO_Pin){
	uint32_t tmp;
	if(GPIO_Pin == USER_IO_A){
		GPIO_INT_Enable_Bits(USER_IO_A_PORT, USER_IO_A_PIN, Reset);
		tmp = pulse_io_a_cnt;
		pulse_io_a_cnt = 0;
		GPIO_INT_Enable_Bits(USER_IO_A_PORT, USER_IO_A_PIN, Set);
	}else if(GPIO_Pin == USER_IO_B){
		GPIO_INT_Enable_Bits(USER_IO_B_PORT, USER_IO_B_PIN, Reset);
		tmp = pulse_io_b_cnt;
		pulse_io_b_cnt = 0;
		GPIO_INT_Enable_Bits(USER_IO_B_PORT, USER_IO_B_PIN, Set);
	}else if(GPIO_Pin == USER_IO_C){
		GPIO_INT_Enable_Bits(USER_IO_C_PORT, USER_IO_C_PIN, Reset);
		tmp = pulse_io_c_cnt;
		pulse_io_c_cnt = 0;
		GPIO_INT_Enable_Bits(USER_IO_C_PORT, USER_IO_C_PIN, Set);
	}else if(GPIO_Pin == USER_IO_D){
		GPIO_INT_Enable_Bits(USER_IO_D_PORT, USER_IO_D_PIN, Reset);
		tmp = pulse_io_d_cnt;
		pulse_io_d_cnt = 0;
		GPIO_INT_Enable_Bits(USER_IO_D_PORT, USER_IO_D_PIN, Set);
	}
	return tmp;
}

void PORTC_IRQ_Handler(void){
	if(GPIO_Read_INTstatus(USER_IO_A_PORT, USER_IO_A_PIN)){
#ifdef _PULSE_DEBUG_
		uint8_t val = GPIO_ReadInputDataBit(USER_IO_A_PORT, USER_IO_A_PIN);
		printf("USER_IO_A_PIN interrupt value %d \r\n", val);
#endif
		GPIO_INT_Clear(USER_IO_A_PORT);
		pulse_io_a_cnt++;
	}else if(GPIO_Read_INTstatus(USER_IO_B_PORT, USER_IO_B_PIN)){
#ifdef _PULSE_DEBUG_
		uint8_t val = GPIO_ReadInputDataBit(USER_IO_B_PORT, USER_IO_B_PIN);
		printf("USER_IO_B_PIN interrupt value %d \r\n", val);
#endif
		GPIO_INT_Clear(USER_IO_B_PORT);
	}else if(GPIO_Read_INTstatus(USER_IO_C_PORT, USER_IO_C_PIN)){
#ifdef _PULSE_DEBUG_
		uint8_t val = GPIO_ReadInputDataBit(USER_IO_C_PORT, USER_IO_C_PIN);
		printf("USER_IO_C_PIN interrupt value %d \r\n", val);
#endif
		GPIO_INT_Clear(USER_IO_C_PORT);
	}else if(GPIO_Read_INTstatus(USER_IO_D_PORT, USER_IO_D_PIN)){
#ifdef _PULSE_DEBUG_
		uint8_t val = GPIO_ReadInputDataBit(USER_IO_D_PORT, USER_IO_D_PIN);
		printf("USER_IO_D_PIN interrupt value %d \r\n", val);
#endif
		GPIO_INT_Clear(USER_IO_D_PORT);
	}
}

/**
  * @brief  I/O Initialize Function
  */
...
#if defined (__USE_USERS_GPIO__)
#ifdef __USE_GPIOA_INPUT_ONLY__
	// Hardwire the Port A as Input, ensuring always is INPUT
	set_user_io_enable(USER_IO_A, IO_ENABLE);
	set_user_io_type(USER_IO_A, IO_DIGITAL);
	set_user_io_direction(USER_IO_A, IO_INPUT);
	set_user_dig_type(USER_IO_A, DIG_PULSE_IN);
#endif
	// just for tests here with timer
//	set_user_io_enable(USER_IO_B, IO_ENABLE);
//	set_user_io_type(USER_IO_B, IO_DIGITAL);
//	set_user_io_direction(USER_IO_B, IO_OUTPUT);
//	init_user_io(USER_IO_B);
	...
	if((user_io_info->user_io_enable & io_sel) == io_sel)
	{
		...
		else
		{
			...
			else
			{
				...
				if(get_user_dig_type(io_sel) == DIG_PULSE_IN){
					printf("io_sel %d configured as Digital Input of Pulse Type\n\r", io_sel);
					GPIO_INT_Configuration(USER_IO_PORT[idx], USER_IO_PIN[idx], Rising);
					USER_IO_PORT[idx]->OUTENCLR = USER_IO_PIN[idx]; // I think this is not necessary
				}
				else
				{
					printf("io_sel %d configured as Normal digital Input\n\r", io_sel);
					// NORMAL DIGITAL INPUT CASE
...
//	init_user_io(io_sel);	// I think this can be removed
	
	return ret;
}

#ifdef __ALLOW_GPIO_PULSE_TYPE__
uint8_t set_user_dig_type(uint8_t io_sel, uint8_t type)
{
	struct __user_io_info *user_io_info = (struct __user_io_info *)&(get_DevConfig_pointer()->user_io_info);
	uint8_t ret = 1;
	uint8_t idx;

	if(type == DIG_PULSE_IN)
	{
		user_io_info->user_dig_type |= io_sel;
		init_user_io(io_sel); // IO reinitialize
	}
	else if(type == DIG_NORMAL)
	{
		user_io_info->user_dig_type &= ~(io_sel);
		init_user_io(io_sel); // IO reinitialize
	}
...
#ifdef __ALLOW_GPIO_PULSE_TYPE__
uint8_t get_user_dig_type(uint8_t io_sel)
{
	struct __user_io_info *user_io_info = (struct __user_io_info *)&(get_DevConfig_pointer()->user_io_info);
	uint8_t ret;

	if((user_io_info->user_dig_type & io_sel) == io_sel)
	{
		ret = DIG_PULSE_IN;
	}
	else
