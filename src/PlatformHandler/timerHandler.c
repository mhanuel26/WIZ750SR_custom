...
static uint16_t tmp_int = 0;
static volatile uint32_t freqVal_io_a = 0;
static volatile uint32_t freqVal_io_b = 0;
static volatile uint32_t freqVal_io_c = 0;
static volatile uint32_t freqVal_io_d = 0;

extern volatile uint8_t pulse_tick;
...
void Timer_Configuration_Counter(void)
{
	...
	Dualtimer_InitStructure.TimerLoad = GetSystemClock() / 256;
	Dualtimer_InitStructure.TimerControl_Mode = DUALTIMER_TimerControl_Periodic;
	Dualtimer_InitStructure.TimerControl_OneShot = DUALTIMER_TimerControl_Wrapping;
	Dualtimer_InitStructure.TimerControl_Pre = DUALTIMER_TimerControl_Pre_256;
	Dualtimer_InitStructure.TimerControl_Size = DUALTIMER_TimerControl_Size_32;
...
void Timer_Freq_Counter_IRQ_Handler(void)
{
	if(DUALTIMER_GetIntStatus(DUALTIMER1_0))
	{
		...
		#if (BASE_BOARD_NAME == WIZ750SR_DEVEL)
		tmp_int ^= 1;
		set_user_io_val(USER_IO_SEL[1], &tmp_int);
		#elif (BASE_BOARD_NAME == WIZ750SR_POE)
		freqVal_io_a = get_pulse_cnt(USER_IO_A);
		pulse_tick = 1;
		#endif
#endif
