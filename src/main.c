...
int main(void)
{
    ...
    while(1) // main loop
    {
        ...
        if(pulse_tick){
        	pulse_tick = 0;
        	uint32_t freqVal = get_io_frequency(USER_IO_A);
        	printf("IO Frequency = %lu Hz\r\n", freqVal);
        }
...
void display_Dev_Info_header(void)
{
    ...
    printf(" %s \r\n", DEVICE_ID_DEFAULT);
    printf(" >> WIZnet Serial to Ethernet Device (PoE Bridge v1.0)\r\n");
#else
    #ifndef __W7500P__
